...
pub async fn compile_nuke(
    ...
    for version in versions {
        ...
        if use_zig {
            binaries.push(compile_zig(&version, target, cuda_backend).await?);
            continue;
        }
        ...
        binaries.push(compile_native(&version, target, cuda_backend).await?);
    }
...
fn collect_features(cuda_backend: bool) -> Vec<String> {
    ...
    if cuda_backend {
        features.push("cuda_backend".to_string());
    }
    features
}
...
async fn compile_native(
    ...
    if target == TargetPlatform::Windows
        && nuke_source_directory(version).join("RIPFramework.dll").exists()
        && !nuke_source_directory(version).join("RIPFramework.lib").exists()
    {
        create_lib_from_dll(&nuke_source_directory(version).join("RIPFramework.dll")).await?;
    }

    let features = collect_features(cuda_backend);
    let mut expression = if features.is_empty() {
        cmd!(
            "cargo",
            "build",
            "--manifest-path",
